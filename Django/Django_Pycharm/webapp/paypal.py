...
def invoice_confirmation(group_id): 
    ...
    for user_contribution in user_contributions:
        ...
        if cost_per_user < user_contribution["contribution"]: #User Over contributed, payout. 
            ...
            payout_id = create_payout(group_id, user_contribution["contribution"]-cost_per_user, user_contribution["user_id"], 
                user_contribution["user_id"]+"-"+group_id)
            if payout_id != None: 
                results ["payouts"] = results ["payouts"] + 1
                payout = models_payout(group_id = group_id, user_id = index, amount = user_contribution["difference"], #TODO change back to user_contribution["user_id"], Hard set
            ...
            if payment_id != None:
                ...
                payment = models_payment(group_id = group_id, user_id = index , amount = user_contribution["difference_non_neg"], #TODO change back to user_contribution["user_id"], Hard set
...
def get_group_payment_statuses(user_id, group_id):
    ...
    status = {"paypal_url": paypal_url, "reimbursement_date": reimbursement_date, "reimbursement_amount": reimbursement_amount, "payment_amount": payment_amount, "payment_date": payment_date, "group_invoiced_date": group_invoiced_date}
